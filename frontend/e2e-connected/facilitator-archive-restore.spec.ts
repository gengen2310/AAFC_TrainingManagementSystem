...
// REM-133: Facilitator archive existed with no restore counterpart, and
// archived facilitators were entirely invisible (no include_archived param,
// no way to even see one to restore it). Added a "Show archived" toggle
// (matching the existing Flight/Wing/Squadron pattern) plus a Restore button.
...
test("an archived facilitator is hidden by default, visible via Show archived, and Restore brings it back", async ({ page }) => {
  ...
  const lastName = `E2E REM-133 ${Date.now()}`;
  const createRes = await page.request.post(`${base}/api/facilitators`, {
    data: { last_name: lastName },
    headers: hdr,
  });
  expect(createRes.ok()).toBe(true);
  const facId = (await createRes.json()).facilitator_id as string;
  const archiveRes = await page.request.delete(`${base}/api/facilitators/${facId}`, { headers: hdr });
  ...
  // Hidden by default.
  await expect(page.locator("#fac-tbody")).not.toContainText(lastName);

  // Visible with "Show archived" checked, flagged as archived.
  await page.locator("#fac-show-archived").check();
  const row = page.locator("#fac-tbody tr", { hasText: lastName });
  await expect(row).toBeVisible({ timeout: 5000 });
  await expect(row).toContainText("Archived");

  // Restore brings it back into the default (unchecked) view.
  await row.getByRole("button", { name: "Restore" }).click();
  await page.locator("#fac-show-archived").uncheck();
  await expect(page.locator("#fac-tbody tr", { hasText: lastName })).toBeVisible({ timeout: 5000 });

  // Cleanup.
  await page.request.delete(`${base}/api/facilitators/${facId}`, { headers: hdr });
});

test("sqn_general (read-only) sees no Show archived control on Facilitators", async ({ page }) => {
  ...
  await expect(page.locator("#fac-show-archived")).toBeHidden();
});
