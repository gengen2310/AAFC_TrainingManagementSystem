...
| D | Strengthen tests | **Started.** Real coverage measured (78% statement / 66% branch overall, `test_coverage_matrix.csv`). Hand-crafted mutation testing on `app/permissions.py` (the highest-blast-radius module): 5/7 killed on first pass, 2 real survivors (QUAL-006 IDOR-class gap, QUAL-007) found, fixed with a new `test_permissions.py`, and verified — both now killed. See `04_backend_review.md`. Not yet done: mutation testing on other critical modules, full static-analysis suite, frontend coverage/mutation testing. |
