...
| C | Correct architecture/integration defects | **C-2/C-3 done** (dead `_session_out()`/`ScheduledSession`/`PlanningLocation` router code removed, `ScheduledSessionIn`/`UpdateIn` renamed to `SessionCreateIn`/`SessionUpdateIn` — zero behaviour change, deployed to staging, verified live). **C-5 explicitly deferred** by user decision (multi-router refactor, not a confirmed defect). C-4/C-6 belong to Phase F (performance); C-7 is a tracked parity item (REM-72); C-8 is explicitly out of Phase C scope. Table/model retirement for the now-fully-dead `scheduled_sessions`/`planning_locations` was offered and explicitly declined by the user for now (QUAL-002). |
| D | Strengthen tests | **Core mutation-testing pass complete.** Real coverage measured (78% statement / 66% branch overall, `test_coverage_matrix.csv`). Hand-crafted mutation testing on all four originally-identified highest-blast-radius modules — `permissions.py` (5/7 → 7/7 after fixing QUAL-006/007), `dependencies.py::get_principal` (3/5 → 4/5, QUAL-008 real gap fixed, QUAL-009 characterized not-a-gap), `security.py` (4/6 → 5/6, QUAL-012 real gap fixed, QUAL-013 characterized not-a-gap), `accounts.py::_require_manage_authority` (3/5 → 5/5, QUAL-014/015 real gaps fixed) — see `04_backend_review.md` §2–2d. This completes Phase D's originally-scoped target list, not Phase D itself: still not done are mutation testing on further modules (`training.py`, `planning.py` write endpoints), a full static-analysis suite, and frontend coverage/mutation testing. |
| E | Adversarial security/reliability | **Started.** Live-verified against staging (never production for active testing): candidate 1 (`ENVIRONMENT`/config gate) — PASS, all 5 `validate_for_production()` checks confirmed passing on live production values; candidate 2 (Proxy/Intervention persistence) — CONFIRMED via a real logout/login/write test, `QUAL-004`, remediation direction pending explicit decision. See `08_adversarial_test_report.md`. 5 of 7 candidates from the security review's list not yet executed (multi-squadron import scope, stored XSS, upload size, per-IP rate limiting, change_role session behaviour). Also unrelated to the security review's own list: `REM-113` (a live P0 found via user report during this program, root cause fixed, deployed to staging then production with explicit authorization, end-to-end verified via live browser session) is the clearest evidence yet that this program's live-testing discipline catches real, user-impacting defects, not just theoretical ones. |
...
| `08_adversarial_test_report.md` | Adversarial QA findings | In progress — 2 of 7 security-review candidates live-verified |
