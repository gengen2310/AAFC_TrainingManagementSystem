...
| DEF-10 | Defect | MEDIUM | DB-backed general rate limiter — in-memory `_api_hits` degrades proportionally with Gunicorn worker count; `GUNICORN_WORKERS ≤ 2` operational cap enforced until Option A (DB-backed) is implemented | Implemented commit `<next>`: `IpApiRequest` model + v38 migration; `check_api_rate_db` / `reset_api_rate_limiter_db` in `security.py`; `main.py` uses DB-backed path when `ENVIRONMENT=production/staging`, in-memory in development; `system.py` reset endpoint clears both. 4 regression tests added to `test_rate_limiting.py`. `GUNICORN_WORKERS` cap lifted. | FIXED LOCALLY |
