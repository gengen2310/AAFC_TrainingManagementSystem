...
| 10 | Incomplete report catalogue | PARTIALLY COMPLETE | 12 report endpoints implemented across 3 tiers (5 squadron, 5 wing, 2 national); Tier 1–3 catalogue documented in `09_report_catalogue.md`; T2-04 (wing-cancellation-trend) and T2-05 (wing-not-delivered) now wired to Wing dashboard cards (`connected-frontend/index.html` — all 5 wing endpoints now rendered) | Export (PDF/CSV) for Tier 1 reports; date-range filter; Tier 3 requires Level B multi-Wing data | Medium | ✅ Tier 1 defined | ✅ Tier 2 all 5 endpoints wired | ✅ Tier 3 defined | See `09_report_catalogue.md` |
...
| 12 | Playwright E2E coverage incomplete | PARTIALLY COMPLETE | 114 tests in 22 Planning Workspace spec files; 152 tests in 37 connected-frontend spec files (`frontend/e2e-connected/`) covering main TMS workflows, wing/national dashboard, training classes, activities, facilitators, accounts, calendar, session status/tags, training dashboard (incl. T2-04/T2-05 wing cards); 17 accessibility tests in `tools/playwright-staging/` | Remaining: multi-Wing scope tests (gated on Level B Wing activation) | High | ✅ core workflows done | ✅ multi-Wing | ✅ Axe (Level C) | — |
| 13 | Accessibility automation | PARTIALLY COMPLETE | `a11y-staging.spec.ts`: 25 tests (all four roles: system_admin, sqn_admin, wing_admin, national_admin — Axe select-name/label/duplicate-id/aria rules + keyboard focus + no-duplicate-IDs); `a11y-wcag.spec.ts`: 9 tests covering SC 1.4.10 Reflow (320 px), SC 1.4.4 Resize Text (640 px/200% zoom simulation), SC 1.4.3 Color Contrast (Axe color-contrast rule, hard-assert WCAG AA); `a11y-local.spec.ts`: 2 tests | Staging tests gated on env vars (not in main CI); Axe not in main `frontend/e2e/` test suite; high-contrast mode not tested | Medium — no AODA/WCAG evidence | — | ✅ | ✅ | — |
...
| 15 | Distributed rate limiting | PARTIALLY COMPLETE | Per-IP sliding window `api_rate_limit` middleware (`main.py:200`) covers ALL non-exempt `/api/` endpoints; DB-backed `IpLoginAttempt` covers login specifically; multi-replica assessment in `15_rate_limiting_assessment.md`: in-memory `_api_hits` degrades 2× per worker (effective limit = 300 × workers), login limiter is shared/exact; **operational cap: `GUNICORN_WORKERS ≤ 2` until DB-backed limiter implemented (Option A in assessment doc)** | Per-account limiting not implemented; DB-backed general rate limiter not yet implemented (Option A from assessment); Redis (Option B) not provisioned | Medium | ✅ done | ✅ implement DB-backed (Option A) before raising GUNICORN_WORKERS | ✅ implement if needed | See `15_rate_limiting_assessment.md` |
...
| 21 | Backup and restore continuous verification | PARTIALLY COMPLETE | Daily backup + weekly restore workflows active; last proven runs documented; GPG public key committed to repo (`.github/backup-public-key.asc`); key custody remaining human steps (private key offline storage, GitHub Secrets, manual backup confirmation) documented in `v1_go_no_go_checklist.md §C`; DR rehearsal procedure written (`19_disaster_recovery_rehearsal.md`) but not yet run | (a) Complete key custody steps in `v1_go_no_go_checklist.md §C`; (b) Run DR rehearsal per `19_disaster_recovery_rehearsal.md`; (c) Record result in rehearsal Evidence Table; (d) Schedule quarterly rehearsal | High — backup workflows exist but not rehearsed end-to-end for a genuine outage | ✅ key custody + DR rehearsal | ✅ | ✅ | — |
| 22 | External penetration testing | PARTIALLY COMPLETE | Scope of work documented in `22_pen_test_scope.md`: grey-box, 5-day engagement, all 7 roles, covers auth/IDOR/session/CSRF/XSS/import abuse/audit integrity/infrastructure/dependency scan; vendor engagement not yet initiated; organisational approval required | Vendor selected, engaged, and engagement completed; Critical and High findings remediated | Critical for National — required before releasing beyond 7WG beta | — | ⚠️ recommended | ✅ required | **MANUAL APPROVAL REQUIRED** — budget and scope from org; see `22_pen_test_scope.md` |
...
| 24 | Beta feedback not collected | PARTIALLY COMPLETE | `docs/next-stage/02_beta_feedback_register.md` template created with classification guide, priority definitions, and item register table | Register must be populated with actual beta feedback from 7WG beta testers; all items classified before V1 release | Medium — systemic bugs may exist that testing did not catch | ✅ register + classification DONE | — | — | Register template exists; items to be populated by beta participants |
...
| PARTIALLY COMPLETE | 13 | 2, 5, 8, 9, 10, 12, 13, 15, 16, 20, 21, 22, 24 |
...
**Consolidated pre-flight checklist (human-action gates):** `docs/next-stage/v1_go_no_go_checklist.md`
Contains all human steps for Gaps 5, 20, 21, 23, and 24 in a single operator-facing document with sign-off lines.
