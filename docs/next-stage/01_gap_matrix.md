...
| 8 | Multi-Wing onboarding not automated | PARTIALLY COMPLETE | `backend/app/seeds/second_wing_seed.py`: idempotent Wing + 2 squadrons + 4 accounts + PlanningYear + 7 Canadian holidays; `DRY_RUN=1` preview mode; structured JSON onboarding report; audit log entries tagged `staging_onboarding`; staging-only (refused in production) | Rollback procedure not yet documented; no multi-Wing Wing-onboarding API (only CLI seed); full TZ field not in Wing model | High | — | ✅ script + runbook | ✅ | See `10_wing_onboarding_runbook.md` |
