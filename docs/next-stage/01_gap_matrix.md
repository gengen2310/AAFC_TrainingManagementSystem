...
| 13 | Accessibility automation | PARTIALLY COMPLETE | `tools/playwright-staging/tests/a11y-staging.spec.ts`: 25 tests total — 16 existing (system_admin + sqn_admin Axe + keyboard) + 9 new (Wing Admin: Wing Overview/Activities/HQ Calendar/wing-dash T2-04 T2-05 cards/keyboard; National Admin: National Overview/Activities/keyboard/cmd-window-sel); `a11y-local.spec.ts`: 2 tests; new tests gated on `STAGING_WING_ADMIN_CODE` / `STAGING_NATIONAL_ADMIN_CODE` env vars | Zoom/contrast tests not written; staging tests gated on env vars (not in main CI); Axe not in main `frontend/e2e/` test suite | Medium — no AODA/WCAG evidence | — | ✅ | ✅ | — |
