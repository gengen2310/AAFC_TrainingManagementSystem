...
| 13 | Accessibility automation | PARTIALLY COMPLETE | `tools/playwright-staging/tests/a11y-staging.spec.ts`: 15 Axe tests (system_admin + sqn_admin; `select-name`, `label`, `duplicate-id`, `aria-allowed-attr` rules; dashboard, system console, accounts, parade nights, weekly program, activities, facilitators); 9 keyboard-only tests (Tab, label click-to-focus, focus() reachability, aria-label presence); `a11y-local.spec.ts`: 2 tests (auth form + duplicate-id full-page); committed `8277803` | Wing admin and national admin pages not Axe-tested; zoom/contrast tests not written; staging tests gated on env vars (not in main CI); Axe not in main `frontend/e2e/` test suite | Medium — no AODA/WCAG evidence | — | ✅ | ✅ | — |
