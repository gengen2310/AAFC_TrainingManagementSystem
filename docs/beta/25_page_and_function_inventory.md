...
| Activities | `activities` | sqn_admin, sqn_general | Review wing/national activities and local entries | Add local activity | `/api/training/activities` | **RETAIN AND IMPROVE** | Import CEA button retired here (DEFECT-005, 2026-07-26) — CEA import is now Planning Workspace-only (`wing_admin`+); `classify` is also a Planning Workspace-only action on `CeaActivity`, not available from this page |
