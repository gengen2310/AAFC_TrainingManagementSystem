...
# ── CEA MEMBER IMPORT (REM-200: upsert on service_number) ───────────────────
...
_CEA_HEADER_ALIASES = {
    # column → list of substrings (case-insensitive) to match
    "service_number": ["id", "cea id", "cadet id", "cadet no", "service"],
    "rank": ["rank"],
    "first_name": ["first", "given"],
    "last_name": ["family", "last", "surname"],
}

def _cea_find_col(headers: list[str], aliases: list[str]) -> int | None:
...
def _parse_cea_rows(csv_text: str) -> tuple[list[str], list[dict]]:
    ...
    if not rows:
        return [], []
    headers = rows[0]
    col_idx = {field: _cea_find_col(headers, aliases)
               for field, aliases in _CEA_HEADER_ALIASES.items()}
    parsed = []
    for row in rows[1:]:
        def get(field):
...
def _classify_cea_rows(db: DBSession, sq_id: str, parsed_rows: list[dict]):
    """Classify each row as NEW | UPDATE | UNCHANGED | ERROR."""
    from ..models.training import Cadet
    result = []
    for i, row in enumerate(parsed_rows):
        ...
        errors = []
        if not sn:
            errors.append("missing service_number (CEA Id)")
        if not ln:
            errors.append("missing last_name (Family name)")
        if errors:
            result.append({
        ...
        if not existing:
            result.append({"row": i + 2, "action": "NEW", **row, "errors": []})
        else:
            # compare rank/name — normalize None to ""
            changed = (
                (existing.rank or "") != row["rank"]
                or (existing.first_name or "") != row["first_name"]
                or (existing.last_name or "") != row["last_name"]
            )
            result.append({
                "row": i + 2,
                "action": "UPDATE" if changed else "UNCHANGED",
...
def cea_member_import_preview(
    ...
    _, parsed = _parse_cea_rows(body.csv_text)
    if not parsed:
        raise HTTPException(400, detail={"error": "no_rows"})

    classified = _classify_cea_rows(db, sq_id, parsed)
    ...
    return {
        ...
        "rows": classified[:200],  # cap preview to 200 rows
    }


@router.post("/import/cea-members/commit")
def cea_member_import_commit(
    body: CeaMemberImportIn,
    db: DBSession = Depends(get_db),
    p: Principal = Depends(get_principal),
):
    from ..permissions import require_can_write_squadron
    from ..models.training import Cadet
    from ..models.operations import CadetMemberImportBatch
    ...
    for row in classified:
        ...
        if row["action"] == "NEW":
            ...
            cadet = db.get(Cadet, row["cadet_id"])
            delta.append({
                "cadet_id": cadet.id,
                "action": "UPDATE",
                "prev_rank": cadet.rank,
                "prev_first_name": cadet.first_name,
                "prev_last_name": cadet.last_name,
            })
            cadet.rank = row["rank"] or None
            cadet.first_name = row["first_name"] or None
            cadet.last_name = row["last_name"]
            cadet.updated_by = p.user_id
            update_count += 1
        else:
            unchanged_count += 1
    ...
    batch = CadetMemberImportBatch(
        ...
        source_file_name=body.source_file_name or "",
        row_count=len(classified),
        new_count=new_count,
        ...
        committed=True,
        import_delta=json.dumps(delta),
    )
    db.add(batch)
    db.commit()
    audit(db, p, object_type="cea_member_import", object_id=batch.id, action="import_commit",
          new={"new": new_count, "updated": update_count, "unchanged": unchanged_count, "errors": error_count})
    return {
        "ok": True,
        "batch_id": batch.id,
        "new_count": new_count,
...
@router.post("/import/cea-members/rollback")
def cea_member_import_rollback(
    batch_id: str,
    db: DBSession = Depends(get_db),
    p: Principal = Depends(get_principal),
):
    ...
    delta = json.loads(batch.import_delta or "[]")
    new_archived = updated_restored = 0
    ...
    for entry in delta:
        ...
        if not cadet:
            continue
        if entry["action"] == "NEW":
            # Soft-archive the cadet that was created
            cadet.is_archived = True
            cadet.archived_at = utcnow()
            new_archived += 1
        elif entry["action"] == "UPDATE":
            # Restore previous rank/name
            cadet.rank = entry["prev_rank"]
            cadet.first_name = entry["prev_first_name"]
            cadet.last_name = entry["prev_last_name"]
            cadet.updated_by = p.user_id
    ...
    batch.rollback_status = "rolled_back"
    db.commit()
    audit(db, p, object_type="cea_member_import", object_id=batch.id, action="import_rollback",
          new={"new_archived": new_archived, "updated_restored": updated_restored})
    return {"ok": True, "batch_id": batch.id, "new_archived": new_archived, "updated_restored": updated_restored}
